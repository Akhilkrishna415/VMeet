...
public class CreatePostActivity extends AppCompatActivity implements DatePickerDialog.OnDateSetListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //validate End Time and Start time when room number is changed and look for conflicts of meetings with rooms..
        roomNoSp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                String selectedroom = parent.getItemAtPosition(position).toString();
                AsyncTaskExample asyncTask = new AsyncTaskExample();
                asyncTask.execute(eventdate.getText().toString(), stime.getSelectedItem().toString(), etime.getSelectedItem().toString(), selectedroom);
...
//                Toast.makeText(getApplicationContext(),room_url[1], Toast.LENGTH_SHORT).show();
                ...
                if (startTimeValue > endTimeValue) {
                    Toast.makeText(getApplicationContext(), "Start time should not be less than End time. " + endTimeValue, Toast.LENGTH_SHORT).show();
...
//            publishProgress("Sleeping..."); // Calls onProgressUpdate()
            try {
//                CollectionReference currentData = firestore.collection("NewRoomRequest");
//
//                currentData.whereEqualTo("event_date", params[0]).whereEqualTo("start_time","").whereEqualTo("end_time","")
//                        .get()
//                        .addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
//                            @Override
//                            public void onComplete(@NonNull Task<QuerySnapshot> task) {
